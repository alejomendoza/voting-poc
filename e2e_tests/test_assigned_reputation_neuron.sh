...
invoke_function $ASSIGNED_REPUTATION_NEURON_ID get_external_data_provider
...
ORACLE_RESULT=$(invoke_function $ASSIGNED_REPUTATION_NEURON_ID oracle_function --voter_id user001 --_project_id project001 --maybe_previous_layer_vote "[4,500]")
echo $ORACLE_RESULT

echo "[*] invoking set_weight"
invoke_function $ASSIGNED_REPUTATION_NEURON_ID set_weight --new_weight "[1,700]"

echo "[*] invoking weight_function"
invoke_function $ASSIGNED_REPUTATION_NEURON_ID weight_function --raw_neuron_vote $ORACLE_RESULT
